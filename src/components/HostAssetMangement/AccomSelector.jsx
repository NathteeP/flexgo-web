...
const AccomSelector = ({accoms}) => {
  const [accomId, setAccomId] = React.useState(0)
  ...
  const handleChange = (e) => {
    ...
    if (accomId === -1) return;
    setAccomId(accomId)
    dispatch(fetchAllRoomByAccomId(accoms[accomId].id))
  };
  ...
  return (
    <FormControl size='small' className='flex border-0'>
      ...
      >
        {accoms?.length >= 1 ? accoms?.map((item,index) => (<MenuItem key={index} value={item.name}>{item.name}</MenuItem>)) : null}
      </Select>
    </FormControl>
  );
};
