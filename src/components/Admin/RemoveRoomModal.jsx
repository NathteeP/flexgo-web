...
import { submitDeleteRoom } from '../../store/slices/hostForm-slice';
import { fetchAllRoomByAccomId } from '../../store/slices/user-slice';
function RemoveRoomModal() {
  const dispatch = useDispatch();
  const { roomId } = useSelector((state) => state.hostForm.room);
  const { roomsList } = useSelector((state) => state.user);
  const handleOnRemove = (e) => {
    e.preventDefault();
    if (roomsList.length === 1) {
      alert(
        'The accommodation must have at least one room. Sorry for the inconvenience'
      );
      dispatch(closeAdminRemoveRoom());
      return;
    }
    dispatch(submitDeleteRoom(roomId));
    dispatch(fetchAllRoomByAccomId(roomsList[0].accomId));
    dispatch(closeAdminRemoveRoom());
  };
  return (
