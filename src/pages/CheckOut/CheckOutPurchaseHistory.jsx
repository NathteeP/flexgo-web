...
import { openReview, closeReview } from '../../store/slices/modal-slice';
...
export default function CheckOutPurchaseHistory() {
  const dispatch = useDispatch();
  const { reservationId } = useParams();
  const { isReviewOpen } = useSelector((state) => state.modal);
  ...
  if (accom && pathname.includes('reservation'))
    handleReservationApproval(reservationId);
    ...
    <div className=' flex flex-col items-center justify-center p-4 mx-16'>
      ...
      <div className='max-w-5xl w-full border-[2px] border-fg-grey/50 shadow-lg rounded-2xl flex flex-col items-center p-8 mt-8 mb-12'>
        <div className='max-w-4xl w-full flex justify-start mb-8'>
          <p className='font-medium mt-2 text-xl text-fg-text-black flex justify-between w-full items-center'>
            Reservation ID: {reservationId}
            <Button onClick={() => dispatch(openReview())}>Review</Button>
          </p>
        </div>
